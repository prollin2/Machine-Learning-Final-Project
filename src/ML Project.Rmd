...
```{r}
library(xgboost)
dtrain <- xgb.DMatrix(data = as.matrix(train_data[, -c(3,4,5,20)]), label =  as.numeric(train_data$log_arr_del15))
# Create test matrix
dtest <- xgb.DMatrix(data = as.matrix(test_data[, -c(3,4,5,20)]), label = as.numeric(test_data$log_arr_del15))
```
